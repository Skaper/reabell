Shader "Custom/RoundProgressBarShader"
{
	Properties
	{
		[PerRendererData] _MainTex("Sprite Texture", 2D) = "white" {}
		_Color("Tint", Color) = (1,1,1,1)
		_Value("Value", Range(0, 360)) = 15
		...
		CGPROGRAM
			...
			{
				fixed4 c = tex2D(_MainTex, IN.texcoord) * 0.2;//IN.color;
				c.rgb *= c.a;

				//-------- Creating arc --------//
				// sector start/end angles
				float startAngle = _Angle - _Arc1;
				float endAngle = _Angle + _Value;
		...
		ENDCG
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				fixed4 c = tex2D(_MainTex, IN.texcoord) * 0.7;//IN.color;
				c.rgb *= c.a;
